...
- **Rate Limiting:** Five mechanisms in use: (1) Better Auth built-in IP limiter — in-process Worker memory, volatile, resets on isolate recycle, correct and intentional; (2) OTP exponential backoff — D1 table `otp_rate_limit`, cleaned nightly by cron; (3) Password-change limiter — D1 table `password_change_limit`, cleaned nightly by cron; (4) In-memory email limiter — module-level Map, expired entries pruned on each write; (5) Upload/reading daily counters — D1 tables `upload_daily_count` and `reading_daily_count`, rows older than 2 days cleaned nightly by cron. KV is not used for rate limiting. Nightly cleanup runs via `GET /api/cron/cleanup-stale-rate-limits` secured by `CRON_SECRET`.
- Notification Polling: Click-to-refresh in v1.
...
- **Property delete cascade rule:** When a property is deleted by the owner, `billing_periods`, `meter_readings`, `bills`, and `bill_photos` are NOT immediately deleted — they form permanent tenant billing history. Only `meter_reading_edits`, `edit_requests`, `custom_charges`, `property_rates`, and `notifications` are hard-deleted immediately. Tenancies are soft-deleted to `status='property_deleted'`. **Deferred full sweep:** `sweepOrphanedPropertyData` is called via `waitUntil` on the owner DELETE and the tenant `DELETE /tenancies/:id` routes. It hard-deletes R2 photos + all DB history (bill_photos, meter_reading_edits, meter_readings, bills, billing_periods, tenancies) only once the property row is gone AND every real tenant (`isOwnerTenancy = false`) has a non-null `deletedByTenantAt`. `archivedByTenantAt` is solely a reversible UI visibility preference (Hide/Restore) and never triggers deletion. Solo-mode properties (no real tenants) sweep immediately on owner delete. The deferred full sweep is the only path that permanently wipes retained billing history.
...
- **Error Boundaries:** Every Astro page with a React `client:load` or `client:only` island component must wrap its top-level component export using the `withErrorBoundary` HOC to ensure render errors show a fallback UI instead of crashing the island silently, and to provide `QueryClientProvider` at the island root.
- **E2E Smoke Tests:** Playwright (`@playwright/test`) with a single `chromium` project. Tests live in `tests/e2e/`. Two spec files: `smoke.spec.ts` (owner flows) and `tenant.spec.ts` (tenant flows). Each test logs in via the `/login` form using `scripts/seed.ts` demo credentials, navigates to a route, asserts a key element is visible, and asserts that `role=heading "Something went wrong"` is NOT visible. Run with `pnpm test:e2e`. CI runs them in the `e2e` job (needs: ci). Workers = 1 (serial) because local D1 is a single SQLite file with no isolation.
- **Migration baseline:** All migrations were squashed into `src/db/migrations/0000_init.sql` (July 2026). This includes `rate_override` on `billing_periods` and all schema up to that date. Future migrations start from `0001`. The `meta/` folder contains `0000_snapshot.json`, `_journal.json`, and a snapshot per post-squash migration that makes schema changes. `0001_integrity_indexes` has no snapshot because it added only indexes (no table/column changes); `0002_snapshot.json` therefore has `prevId` pointing directly to `0000`. Drizzle always uses the latest snapshot file for future `drizzle-kit generate` calls — missing intermediate snapshots are safe.

- **Data Fetching:** All client-side server-data fetching uses TanStack Query v5. `QueryClientProvider` is injected by `withErrorBoundary` or `QueryProvider` at island roots. Query keys are defined in `src/lib/query-keys.ts`. Do not use raw `useEffect`+`useState` for server data.
...
- **Edit request throttle after overwrite:** Edit request rate limits are applied per-tenant per-property. The throttle check runs _first_ but subtracts any existing same-period pending request from the total count. Only after passing the throttle is the existing request cancelled and overwritten. This ensures strict upper bounds without penalizing same-period correction updates.
...
- **E2E job:** Runs on every PR/push that touches `src/**`, `scripts/**`, `tests/**`, `package.json`, `pnpm-lock.yaml`, `tsconfig.json`, `eslint.config.mjs`, `vitest.config.mts`, `astro.config.mjs`, `wrangler.jsonc`, `drizzle.config.ts`, `playwright.config.ts`, or `.github/workflows/ci.yml`. Needs `ci` job to pass first. Seeds local D1 with `pnpm seed:fresh`, then runs `pnpm test:e2e`. Uploads `playwright-report/` as an artifact on failure (7-day retention).
...
- **Dependency security:** CI runs `pnpm audit --prod` on pushes to main. Remediation policy: upgrade direct dep → `pnpm.overrides` for transitive → `ignoreGhsas` only when no compatible fix exists. Per-GHSA rationale for current `ignoreGhsas` entries:
  ...
  - `GHSA-2p49-hgcm-8545` (svgo): prod transitive via `astro > svgo`; no compatible fix in the current astro 7.x line; low exploitability in server-only SVG processing path.
  - `GHSA-f88m-g3jw-g9cj` (sharp): prod transitive via `astro > sharp`; no compatible fix in the current astro 7.x line; low exploitability in server-only image processing path.
