...
describe("email.ts", () => {
  ...
  describe("Resend provider", () => {
    ...
    it("throws when RESEND_API_KEY is missing", async () => {
      ...
      await expect(sendEmail(env, payload)).rejects.toThrow(
    ...
    it("throws with Resend error message when Resend returns { error }", async () => {
      ...
      await expect(sendEmail(env, payload)).rejects.toThrow(
        "Failed to send email via Resend: Resend mock error"
      );
    });
  });
  ...
  describe("Atlas provider", () => {
    ...
    it("sends correct JSON body", async () => {
      ...
      server.use(
        http.post(
          "https://test-mailer.workers.dev/send",
          async ({ request }) => {
    ...
    it("throws ATLAS_QUOTA_EXHAUSTED when response is 429", async () => {
      ...
      await expect(sendEmail(atlasEnv, payload)).rejects.toThrow(
        "ATLAS_QUOTA_EXHAUSTED"
      );
    });
    ...
    it("throws when ATLAS_MAILER_URL is missing", async () => {
      ...
      await expect(sendEmail(env, payload)).rejects.toThrow(
        "ATLAS_MAILER_URL or ATLAS_MAILER_SECRET is not set"
      );
    });
  ...
  describe("No provider", () => {
    it("throws in production with descriptive config error message", async () => {
      ...
      await expect(sendEmail(env, payload)).rejects.toThrow(
        "No email provider configured. Set EMAIL_PROVIDER=resend or EMAIL_PROVIDER=atlas."
      );
    });
  ...
  describe("checkEmailRateLimit", () => {
    ...
    it("prunes expired entries from the map on write", () => {
      vi.useFakeTimers();
      const user1 = "prune-user-1";
      const user2 = "prune-user-2";
      const user3 = "prune-user-3";

      checkEmailRateLimit(user1);
      checkEmailRateLimit(user2);

      // Advance past window so user1 and user2 expire
      vi.advanceTimersByTime(60001);

      // Trigger write for user3 — should prune user1 and user2
      expect(checkEmailRateLimit(user3)).toBe(true);

      // Now user1 should be allowed immediately as a fresh write
      expect(checkEmailRateLimit(user1)).toBe(true);
