...
app.post("/api/auth/email-otp/check-verification-otp", async (c) => {
  ...
  if (response.status === 200) {
    try {
      ...
      console.error(
        "[Auth] Failed to reset OTP rate limit after verification:",
        err
      );
    }
  }
...
app.post("/api/auth/change-password", async (c) => {
  ...
  const db = getDb(c.env.DB);
  const { allowed, remainingSeconds } =
    await checkAndIncrementPasswordChangeLimit(db, session.user.id);
  if (!allowed) {
    ...
    c.executionCtx.waitUntil(
      (async () => {
        try {
          ...
          logger.info(
            { userId: user.id, event: "user.password_changed" },
...
app.post("/api/auth/sign-in/social", async (c) => {
  ...
  if (c.env.ENVIRONMENT !== "production") {
    logger.debug(
      {
        urlPreview,
        hasGoogle: !!c.env.GOOGLE_CLIENT_ID,
        hasGithub: !!c.env.GITHUB_CLIENT_ID,
      },
...
app.doc("/api/docs/openapi.json", {
  ...
  info: {
    ...
    version: "1.0.0",
    description:
      "Transparent utility billing platform API. All endpoints require an active session cookie except where noted.",
  },
  servers: [
