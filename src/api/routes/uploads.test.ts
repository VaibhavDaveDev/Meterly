...
import { properties, user, billingPeriods, tenancies, bills, billPhotos } from '../../db/schema';
...
describe('Uploads API', () => {
  ...
  const mockEnv = {
    ...
    BILL_PHOTOS: { put: vi.fn().mockResolvedValue({}), delete: vi.fn().mockResolvedValue({}) }
  };
  ...
  it('rejects upload from non-owner', async () => {
    ...
    const res = await app.request('/uploads/bill-photo', {
      method: 'POST',
      body: formData
  ...
  it('stores photo in R2 and inserts bill_photos row with version 1', async () => {
    ...
    const body = (await res.json()) as { success: boolean; data: { objectKey: string } };
    expect(body.success).toBe(true);
    expect(body.data.objectKey).toBeDefined();

    // Verify DB insert
    const photos = await testDb.select().from(billPhotos).where(eq(billPhotos.objectKey, body.data.objectKey));
