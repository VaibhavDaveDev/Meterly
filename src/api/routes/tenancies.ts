...
const tenanciesRouter = new OpenAPIHono<{
  Bindings: Bindings;
  Variables: Variables;
}>();
...
tenanciesRouter.openapi(getTenanciesRoute, async (c) => {
  ...
  const past = all.filter(
    (t) => t.status === "inactive" || t.status === "declined"
  );

  return c.json(
    { success: true as const, data: { active, invited, past } },
    200
  );
});
...
tenanciesRouter.openapi(inviteTenantRoute, async (c) => {
    ...
    return c.json(
      {
        ...
        error: {
          ...
          message: "Please wait a minute before sending another invite",
        },
      },
      429
    );
  ...
  const [existingActive] = await db
    ...
    .where(
      and(
        ...
        or(eq(tenancies.status, "active"), eq(tenancies.status, "invited"))
      )
    )
    .limit(1);
  ...
  if (existingActive) {
    ...
    if (!isExpired) {
      ...
      return c.json(
        {
          success: false as const,
          error: { code: "TENANCY_EXISTS", message: msg },
        },
        409
      );
    }
  ...
  return c.json(
    { success: true as const, data: { tenancyId, inviteToken } },
    201
  );
});
    ...
    return c.json(
      {
        success: false as const,
        error: {
          code: "CANNOT_REMOVE_OWNER_TENANCY",
          message:
            "Cannot remove the owner tenancy. Toggle solo mode off instead.",
        },
      },
      400
    );
    ...
    return c.json(
      {
        ...
        error: {
          ...
          message: "This tenant has already been removed.",
        },
      },
      409
    );
...
tenanciesRouter.openapi(updateSplitsRoute, async (c) => {
    ...
    return c.json(
      {
        ...
        error: {
          code: "INVALID_SUM",
          message: "Splits must sum to exactly 100%",
        },
      },
      400
    );
  ...
  for (const [id, percentage] of Object.entries(splits)) {
    await db
      .update(tenancies)
      .set({ splitPercentage: percentage })
      .where(and(eq(tenancies.id, id), eq(tenancies.propertyId, propertyId)));
  }

  return c.json(
    { success: true as const, message: "Splits updated successfully" },
    200
  );
});
