...
const ratesRouter = new OpenAPIHono<{
  Bindings: Bindings;
  Variables: Variables;
}>();
      ...
      return c.json(
        {
          ...
          error: {
            code: "UNAUTHORIZED",
            message: "Not authorized to view rates for this property",
          },
        },
        403
      );
    ...
    return c.json(
      {
        ...
        error: {
          ...
          message: "No effective rates found for this property",
        },
      },
      404
    );
...
ratesRouter.openapi(createRateRoute, async (c) => {
    ...
    return c.json(
      {
        ...
        error: {
          code: "UNAUTHORIZED",
          message: "Only the property owner can update rates",
        },
      },
      403
    );
  ...
  const [newRate] = await db
    .select()
    .from(propertyRates)
    .where(eq(propertyRates.id, rateId))
    .limit(1);
    ...
    if (t.tenantId) {
      c.executionCtx.waitUntil(
        createNotification(
          ...
          "rate_changed",
          "Rates Updated",
          `Rates for ${property.name} have been updated.`,
          { propertyId }
        )
      );
    }
    ...
    return c.json(
      {
        ...
        error: {
          code: "UNAUTHORIZED",
          message: "Only the property owner can delete rates",
        },
      },
      403
    );
    ...
    return c.json(
      {
        success: false as const,
        error: {
          code: "CANNOT_DELETE_LAST_RATE",
          message: "Cannot delete the only rate for a property",
        },
      },
      400
    );
