...
const periodsRouter = new OpenAPIHono<{
  Bindings: Bindings;
  Variables: Variables;
}>();
...
const CreatePeriodSchema = z.object({
  periodMonth: z
    .string()
    .regex(/^\d{4}-\d{2}-01$/, "Must be YYYY-MM-01")
...
periodsRouter.openapi(createPeriodRoute, async (c) => {
    ...
    return c.json(
      {
        ...
        error: {
          code: "UNAUTHORIZED",
          message: "Only the property owner can create billing periods",
        },
      },
      403
    );
  ...
  const [existing] = await db
    ...
    .where(
      and(
        ...
        eq(billingPeriods.periodMonth, data.periodMonth)
      )
    )
    .limit(1);
  if (existing) {
    return c.json(
      {
        ...
        error: {
          ...
          message: "A billing period for this month already exists",
        },
      },
      400
    );
  ...
  await db.insert(billingPeriods).values({
    ...
    status: "draft",
    oneOffCharges: data.oneOffCharges
  ...
  if (data.readings) {
    ...
    const activeCharges = (
      await db
        ...
        .where(
          and(
            eq(customCharges.propertyId, propertyId),
