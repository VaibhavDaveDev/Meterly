...
describe("Meter Readings API", () => {
  ...
  describe("POST /:id/readings — Submit reading", () => {
    ...
    it("moves period to pending_approval when readingsRequireApproval is true", async () => {
      await testDb
        .update(properties)
        .set({ readingsRequireApproval: true })
  ...
  describe("Reading submission edge cases", () => {
    it("rejects new POST submissions when period is pending_approval", async () => {
      // Test 3
      await testDb.insert(meterReadings).values({
        id: crypto.randomUUID(),
        billingPeriodId: bpId,
    ...
    it("returns 409 not 500 when UNIQUE constraint race occurs", async () => {
      // Test 4
      await testDb
        .update(billingPeriods)
        .set({ status: "draft" })
    ...
    it("clears submittedBy and submittedAt on reject", async () => {
      // Test 2
      await testDb
        .update(billingPeriods)
        .set({
      ...
      const [bp] = await testDb
        ...
        .from(billingPeriods)
        .where(eq(billingPeriods.id, bpId));
      expect(bp?.submittedBy).toBeNull();
      expect(bp?.submittedAt).toBeNull();
    });
    ...
    it("reopens to submitted status when period was confirmed", async () => {
