...
import { applyApprovedReadings } from "../lib/edit-request-actions";
import {
  SuccessResponse,
  ErrorResponse,
  IdParam,
...
async function loadRequestContext(
  ...
  const periodIds = [...new Set(requests.map((r) => r.billingPeriodId))];
  const readings = periodIds.length
    ? await db
        .select()
        .from(meterReadings)
  ...
  const reviewers = reviewerIds.length
    ...
    : [];
  return {
    readingsMap: new Map(readings.map((r) => [r.billingPeriodId, r])),
    reviewersMap: new Map(reviewers.map((r) => [r.id, r.name])),
  };
}
...
function formatEditRequest(
  ...
  try {
    proposedValues = JSON.parse(request.proposedValues || "{}");
  } catch (e) {
    console.error("Failed to parse proposedValues", request.id, e);
  }
  ...
  if (reading) {
    ...
    const newImport =
      (proposedValues.importEnd ?? reading.importEnd) -
      (reading.importStart || 0);
  ...
  return {
    ...
    reviewedByName: request.reviewedBy
      ? reviewersMap.get(request.reviewedBy)
      : null,
...
const requestsRouter = new OpenAPIHono<{
  Bindings: Bindings;
  Variables: Variables;
}>();
...
requestsRouter.openapi(createEditRequestRoute, async (c) => {
    ...
    return c.json(
      {
        ...
        error: {
          code: "UNAUTHORIZED",
          message: "Only active tenants can request edits",
        },
      },
      403
    );
    ...
    return c.json(
      {
        ...
        error: {
          ...
          message:
            "This billing period is still open. You can edit readings directly from the property overview instead of submitting a correction request. Correction requests are only needed for periods your landlord has already confirmed.",
        },
      },
      400
    );
  ...
  const [currentReading] = await db
    .select()
    .from(meterReadings)
    .where(eq(meterReadings.billingPeriodId, billingPeriodId))
    .limit(1);

  if (currentReading) {
    const pv = data.proposedValues;
    const hasChange =
      (pv.importEnd !== undefined &&
        pv.importEnd !== currentReading.importEnd) ||
      (pv.exportEnd !== undefined &&
    ...
    if (property.hasSolar) {
      const solarGenerated =
        (pv.solarGenerationEnd ?? currentReading.solarGenerationEnd) -
        (currentReading.solarGenerationStart || 0);
      const gridExported =
        (pv.exportEnd ?? currentReading.exportEnd) -
...
requestsRouter.openapi(getEditRequestsRoute, async (c) => {
  ...
  const { readingsMap, reviewersMap } = await loadRequestContext(
    db,
    requestsData.map((r) => r.request)
  );

  const formattedRequests = requestsData.map(
    ({
      request,
      periodMonth,
      periodStatus,
  ...
  if (statusParam === "pending") {
    return c.json(
      {
        success: true as const,
        data: { pending, resolvedCount: resolved.length },
      },
      200
    );
  }
...
requestsRouter.openapi(getPropertyEditRequestsRoute, async (c) => {
  ...
  const formattedRequests = requestsData.map(
    ({ request, periodMonth, periodStatus, requestedByName }) => {
      const reading = readingsMap.get(request.billingPeriodId);
      return formatEditRequest(
        request,
        periodMonth,
...
requestsRouter.openapi(reviewRequestRoute, async (c) => {
  ...
  const [period] = await db
    .select()
    .from(billingPeriods)
    .where(eq(billingPeriods.id, request.billingPeriodId))
    .limit(1);
  if (!period) {
    return c.json(
      {
    ...
    return c.json(
      {
        ...
        error: {
          code: "UNAUTHORIZED",
          message: "Only the property owner can review this request",
        },
      },
      403
    );
  ...
  if (data.action === "reject") {
    ...
    const proposedList = [
      typeof proposedValues.importEnd === "number"
        ? `• Import: ${proposedValues.importEnd} units`
        : null,
      typeof proposedValues.exportEnd === "number"
        ? `• Export: ${proposedValues.exportEnd} units`
        : null,
      typeof proposedValues.solarGenerationEnd === "number"
        ? `• Solar: ${proposedValues.solarGenerationEnd} units`
        : null,
    ]
      .filter(Boolean)
      .join("\n");
    ...
    const currentList = reading
      ? [
          typeof reading.importEnd === "number"
            ? `• Import: ${reading.importEnd} units`
            : null,
          typeof reading.exportEnd === "number"
            ? `• Export: ${reading.exportEnd} units`
            : null,
          typeof reading.solarGenerationEnd === "number"
            ? `• Solar: ${reading.solarGenerationEnd} units`
            : null,
        ]
          .filter(Boolean)
          .join("\n")
...
If you believe this is an error, you can submit a new correction request with additional explanation.`;
  ...
  const [reading] = await db
    ...
    .limit(1);
  const result = await applyApprovedReadings({
    db,
    reading,
    proposedValues,
    property,
  ...
  if (!result.success) {
    if (result.code === "READING_NOT_FOUND") {
      return c.json(
        {
          success: false as const,
    ...
    return c.json(
      {
        ...
        error: {
          code: result.code,
          message: result.error || "Cannot approve.",
        },
      },
      400
    );
  ...
  const cancelledRequests = await db
    ...
    .set({
      status: "cancelled",
      reviewNote:
        "Another edit request for this period was approved. Your request has been cancelled. Please review the updated readings and submit a new request if needed.",
      reviewedAt: new Date(),
      reviewedBy: user.id,
    })
    .where(
    ...
    if (req.requestedBy !== request.requestedBy) {
      c.executionCtx.waitUntil(
        createNotification(
          ...
          `Your edit request for ${period.periodMonth} was cancelled because the owner approved another tenant's request.`,
          { periodId: period.id }
        )
      );
    }
  ...
  c.executionCtx.waitUntil(
    createNotification(
      ...
      `Your edit request for ${period.periodMonth} was approved and the bill has been recalculated.`,
      { requestId }
    )
  );
  ...
  return c.json(
    {
      success: true as const,
      message: "Request approved and recalculation queued",
    },
    200
  );
});
