...
const chargesRouter = new OpenAPIHono<{
  Bindings: Bindings;
  Variables: Variables;
}>();
...
const CustomChargeSchema = z.object({
  name: z
    ...
    .max(100, "Charge name too long (max 100 characters)")
    .openapi({ example: "Internet" }),
    ...
    return c.json(
      {
        ...
        error: {
          code: "UNAUTHORIZED",
          message: "Only the property owner can manage custom charges",
        },
      },
      403
    );
    ...
    if (t.tenantId) {
      c.executionCtx.waitUntil(
        createNotification(
          ...
          `A new recurring charge "${newCharge.name}" has been added for ${property.name}.`,
          { propertyId }
        )
      );
    }
...
chargesRouter.openapi(updateChargeRoute, async (c) => {
  ...
  if (!charge) {
    return c.json(
      {
        success: false as const,
        error: { code: "CHARGE_NOT_FOUND", message: "Custom charge not found" },
      },
      404
    );
  }
    ...
    return c.json(
      {
        ...
        error: {
          code: "UNAUTHORIZED",
          message: "You do not have permission to edit this charge",
        },
      },
      403
    );
    ...
    if (t.tenantId) {
      c.executionCtx.waitUntil(
        createNotification(
          ...
          t.tenantId,
          "charge_updated",
          "Recurring Charge Updated",
          `The recurring charge "${updatedCharge.name}" has been updated for ${property.name}.`,
