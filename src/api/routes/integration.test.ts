...
vi.mock("../middleware/auth", () => ({
  authMiddleware: async (
    ...
    const email =
      userId === "owner-id" ? "owner@example.com" : "tenant@example.com";
...
describe("Integration Flows", () => {
  ...
  const requestAs = async (
    ...
    if (response.status >= 400) {
      ...
      try {
        ...
        console.error(
          `Request Failed: ${method} ${path} -> ${response.status} (non-JSON body)`
        );
      }
    }
    return response;
  ...
  it("Flow 1: Create property -> invite -> accept -> submit -> bill -> pay", async () => {
    ...
    let res = await requestAs(
      "/api/properties",
      "POST",
      { name: "Main House", hasSolar: false },
      "owner-id"
    );
    ...
    res = await requestAs(
      `/api/properties/${propertyId}/tenancies/invite`,
      "POST",
      { email: "tenant@example.com" },
      "owner-id"
    );
    ...
    const activeTenancies = await testDb
      .select()
      .from(tenancies)
    ...
    res = await requestAs(
      `/api/properties/${propertyId}/periods`,
      "POST",
    ...
    res = await requestAs(
      `/api/bills/${tenantBill.billId}/mark-paid`,
      "PATCH",
      null,
      "owner-id"
    );
  ...
  it("Flow 2: Edit reading -> cascade recalc -> verify bills updated", async () => {
    ...
    const resProp = await requestAs(
      "/api/properties",
      "POST",
      { name: "Cascade House", hasSolar: false },
      "owner-id"
    );
    ...
    await requestAs(
      `/api/properties/${propertyId}/rates`,
      "POST",
      { consumptionRate: 10, exportRate: 0, effectiveFrom: "2020-01-01" },
      "owner-id"
    );
    const resTenant = await requestAs(
      `/api/properties/${propertyId}/tenancies/invite`,
      "POST",
      { email: "tenant@example.com" },
      "owner-id"
    ...
    await requestAs(
      `/api/invites/${((await resTenant.json()) as any).data.inviteToken}/accept`,
      "POST",
      null,
      "tenant-id"
    );
    ...
    await requestAs(
      `/api/periods/${pJan}/readings`,
      "POST",
      { solarGenerationEnd: 0, exportEnd: 0, importEnd: 100 },
      "tenant-id"
    );

    // 4. Submit Feb reading: 100 -> 250 (150 consumed)
    await requestAs(
      `/api/periods/${pFeb}/readings`,
      "POST",
      { solarGenerationEnd: 0, exportEnd: 0, importEnd: 250 },
      "tenant-id"
    );
    ...
    let janBill = billsRes.data.bills.find(
      (b: any) => b.periodMonth === "2025-01-01"
    );
    let febBill = billsRes.data.bills.find(
      (b: any) => b.periodMonth === "2025-02-01"
    );
    ...
    await requestAs(
      ...
      "PATCH",
      {
        ...
        exportEnd: 0,
        importEnd: 80,
        reason: "Correcting mistyped reading",
      },
      "owner-id"
    );
    ...
    janBill = billsRes.data.bills.find(
      (b: any) => b.periodMonth === "2025-01-01"
    );
    febBill = billsRes.data.bills.find(
      (b: any) => b.periodMonth === "2025-02-01"
    );
  ...
  it("Flow 3: Solo mode -> verify guard blocks when tenants exist", async () => {
    ...
    let res = await requestAs(
      "/api/properties",
      "POST",
      { name: "Solo House", hasSolar: false },
      "owner-id"
    );
    ...
    res = await requestAs(
      `/api/properties/${propertyId}/mode`,
      "PATCH",
      { soloMode: true },
      "owner-id"
    );
  ...
  it("Flow 4: Owner validation mode", async () => {
    ...
    let res = await requestAs(
      "/api/properties",
      "POST",
      { name: "Strict House", hasSolar: false },
      "owner-id"
    );
    const propertyId = ((await res.json()) as any).data.id;
