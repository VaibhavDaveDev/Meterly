...
describe("Properties API", () => {
  ...
  it("should list empty properties initially", async () => {
    ...
    const body = (await res.json()) as {
      data: { owned: unknown[]; tenant: unknown[]; tenantPast: unknown[] };
    };
  ...
  it("should create a property successfully", async () => {
    ...
    const body = (await res.json()) as {
      success: boolean;
      data: { name: string; hasSolar: boolean; ownerId: string };
    };
  ...
  it("deletes property and cascades correctly", async () => {
    ...
    const propsResult = await testDb
      .select()
      .from(properties)
    ...
    const bpResult = await testDb
      .select()
      .from(billingPeriods)
      .where(eq(billingPeriods.propertyId, id));
    expect(bpResult.length).toBe(1);

    // Operational data (meter readings) is KEPT
    const mrResult = await testDb
      .select()
      .from(meterReadings)
      .where(eq(meterReadings.billingPeriodId, "bp-1"));
    expect(mrResult.length).toBe(1);

    // Tenancies are soft-deleted (status 'property_deleted'), not hard-deleted
    const tenanciesResult = await testDb
      .select()
      .from(tenancies)
