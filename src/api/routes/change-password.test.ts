...
describe("Change Password Route Interception", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(checkAndIncrementPasswordChangeLimit).mockResolvedValue({
      allowed: true,
      remainingSeconds: 0,
    });
  });
  ...
  it("should delegate to Better Auth handler and send email on 200 success", async () => {
    ...
    const sendEmailCallArgs = vi.mocked(sendEmail).mock.calls[0][1];
    expect(sendEmailCallArgs.to).toBe("test@example.com");
    expect(sendEmailCallArgs.subject).toBe(
      "Your Meterly password has been changed"
    );
    expect(sendEmailCallArgs.html).toContain(
      "This is a confirmation that the password for your Meterly account has been successfully changed"
    );
  });
  ...
  it("should return 429 if password change limit is exceeded", async () => {
    ...
    vi.mocked(checkAndIncrementPasswordChangeLimit).mockResolvedValueOnce({
      allowed: false,
    ...
    expect(json.error).toBe(
      "Password change limit reached. Try again in 1 hour."
    );
    expect(mockAuthHandler).not.toHaveBeenCalled();
    expect(sendEmail).not.toHaveBeenCalled();
  });
