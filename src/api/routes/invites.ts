...
const invitesRouter = new OpenAPIHono<{
  Bindings: Bindings;
  Variables: Variables;
}>();
...
invitesRouter.openapi(getInviteRoute, async (c) => {
    ...
    return c.json(
      {
        ...
        error: {
          ...
          message: "This invite was already declined.",
        },
      },
      410
    );
  ...
  const isExpired = row.inviteExpiresAt
    ...
    return c.json(
      {
        ...
        error: {
          code: "INVITE_EXPIRED",
          message: "This invite has expired. Ask the owner to send a new one.",
        },
      },
      410
    );
  ...
  if (property) {
    c.executionCtx.waitUntil(
      createNotification(
        ...
        "tenant_accepted",
        "Invite Accepted",
        `${user.email} has joined ${property.name} as a tenant.`,
        { tenancyId: tenancy.id, propertyId: property.id }
      )
    );
  }
...
invitesRouter.openapi(declineInviteRoute, async (c) => {
  ...
  if (property) {
    sendEmail(c.env, {
      ...
      console.error(
        "[invites/decline] Failed to send decline notification:",
        err
      )
    );
  }
...
invitesRouter.openapi(cancelInviteRoute, async (c) => {
  ...
  const [property] = await db
    ...
    .where(
      and(
        eq(properties.id, tenancy.propertyId),
    ...
    return c.json(
      {
        ...
        error: {
          code: "UNAUTHORIZED",
          message: "Only the property owner can cancel invites.",
        },
      },
      403
    );
