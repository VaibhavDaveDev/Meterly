...
describe("Error Path HTTP Tests", () => {
  ...
  it("POST /api/properties/:id/periods -> 403 when property does not exist", async () => {
    const res = await requestAs(
      "/api/properties/fake-id/periods",
      "POST",
  ...
  it("POST /api/periods/:id/readings -> 404 when period does not exist", async () => {
    const res = await requestAs(
      "/api/periods/fake-period-id/readings",
      "POST",
  ...
  it("POST /api/periods/:id/readings -> 403 when requester is third-party", async () => {
    const resProp = await requestAs(
      "/api/properties",
      "POST",
      { name: "Test Prop", hasSolar: false },
      "owner-id"
    );
    const propId = ((await resProp.json()) as { data: { id: string } }).data.id;

    const pRes = await requestAs(
      `/api/properties/${propId}/periods`,
      "POST",
  ...
  it("GET /api/bills/:id -> 403 when accessed by non-owner, non-tenant user", async () => {
    ...
    // Add Rates
    await requestAs(
      `/api/properties/${propId}/rates`,
      "POST",
      { effectiveFrom: "2025-01-01", consumptionRate: 0.25, exportRate: 0.05 },
      "owner-id"
    );

    // Add tenant
    const resTenant = await requestAs(
      `/api/properties/${propId}/tenancies/invite`,
      "POST",
      { email: "tenant@example.com" },
      "owner-id"
    );
    const inviteToken = (
      (await resTenant.json()) as { data: { inviteToken: string } }
    ).data.inviteToken;
    await requestAs(
    ...
    // Get Bill ID
    const billsRes = await requestAs(
      `/api/properties/${propId}/bills`,
      "GET",
      null,
      "owner-id"
    );
    const billsData = (await billsRes.json()) as {
      data: { bills: Array<{ tenants: Array<{ billId: string }> }> };
    };

    if (!billsData.data.bills[0] || !billsData.data.bills[0].tenants[0]) {
      throw new Error(
        "Bill was not generated. Response: " + JSON.stringify(billsData)
      );
    }
