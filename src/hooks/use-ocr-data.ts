...
export function useOcrData({
  ...
  const handleOcrExtracted = (extractedData: ExtractedData) => {
    ...
    const checkAndPropose = (
      ...
      const currentValNum =
        currentValueStr.trim() === "" ? undefined : parseFloat(currentValueStr);
    ...
    if (extractedData.type === "pdf" && extractedData.pdfResult) {
      checkAndPropose(
        "importEnd",
        "Import Reading",
        extractedData.pdfResult.presentReadingImport,
        importEnd,
        setImportEnd
      );
      if (isSolar) {
        checkAndPropose(
          "exportEnd",
          "Export to Grid",
          extractedData.pdfResult.presentReadingExport,
          exportEnd,
          setExportEnd
        );
        checkAndPropose(
          "solarGenerationEnd",
          "Solar Generation",
          extractedData.pdfResult.presentReadingSolarGen,
          solarGenerationEnd,
          setSolarGenerationEnd
        );
      }
      ...
      checkAndPropose(
        "importEnd",
        "Import Reading",
        extractedData.imageResult.value,
        importEnd,
        setImportEnd
      );
    }
  ...
  const handleResolveConflicts = (
    resolutions: Record<string, "keep_mine" | "use_ocr">
  ) => {
    for (const proposal of pendingProposals) {
      if (resolutions[proposal.field] === "use_ocr") {
        if (proposal.field === "importEnd")
          setImportEnd(String(proposal.ocrValue));
        if (proposal.field === "exportEnd")
          setExportEnd(String(proposal.ocrValue));
        if (proposal.field === "solarGenerationEnd")
          setSolarGenerationEnd(String(proposal.ocrValue));
      }
    }
    setPendingProposals([]);
  };
