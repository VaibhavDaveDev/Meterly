...
export function usePropertyDetails({
  ...
  const handleInvite = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    const { error } = await apiClient.post(
      `/properties/${property.id}/tenancies/invite`,
      payload
    );
  ...
  const handleStartPeriod = async (
    ...
    const { error } = await apiClient.post(
      `/properties/${property.id}/periods`,
      { periodMonth }
    );
    ...
    toast({
      title: "Billing Period Started",
      description: "You can now submit meter readings for this month.",
    });
  ...
  const availableYears = useMemo(() => {
    ...
    const years = new Set(
      billsData.bills.map((p) =>
        new Date(p.periodMonth).getFullYear().toString()
      )
    );
  ...
  const downloadCsv = () => {
    ...
    billsData.bills.forEach((p) => {
      const monthLabel = new Date(p.periodMonth).toLocaleString("default", {
        month: "long",
        year: "numeric",
      });
      p.tenants.forEach((t) => {
  ...
  const chartData = useMemo(() => {
    ...
    const modeChangeMonth = property.soloModeChangedAt
      ? new Date(property.soloModeChangedAt).toLocaleString("default", {
          month: "short",
          year: "2-digit",
        })
      : null;
    ...
    return [...billsData.bills]
      ...
      .slice(-12)
      .map((p) => {
        const label = new Date(p.periodMonth).toLocaleString("default", {
          month: "short",
          year: "2-digit",
        });
