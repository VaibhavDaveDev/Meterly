...
export function validateFilePreFlight(file: File): string | null {
  ...
  if (isPdf) {
    if (file.size > 10 * 1024 * 1024)
      return "PDF is too large. Maximum is 10 MB.";
  } else {
    ...
    if (file.size > 20 * 1024 * 1024)
      return "Image is too large. Maximum is 20 MB before compression.";
  }
...
export async function processPdfFile(file: File): Promise<{
  ...
  try {
    ...
    const message =
      e instanceof Error && e.message ? e.message : "Failed to process PDF.";
  ...
  if (pdfResult.error === "PASSWORD_PROTECTED") {
    return {
      success: false,
      message:
        "This PDF is password-protected. Open it in a PDF viewer, remove the password, then upload again.",
    };
  }
  if (pdfResult.error === "CORRUPT_PDF") {
    return {
      success: false,
      message:
        "This file could not be read as a PDF. It may be corrupted or incomplete.",
    };
  }
  ...
  return {
    success: true,
    message:
      pdfResult.extractionMethod !== "none"
        ? "PDF processed successfully"
        : "Could not extract readings from PDF. Enter manually.",
    result: pdfResult,
  };
}
  ...
  try {
    ...
    if (errorMsg.includes("too small")) {
      ...
      return {
        success: false,
        message:
          "HEIC format is not supported by your browser. Please convert to JPEG.",
      };
    } else {
      return {
        success: false,
        message: "Failed to compress image. Try a different file.",
      };
    }
  }
}
