...
export function PropertyRatesTable({
  propertyId,
  isOwner,
}: PropertyRatesProps) {
  ...
  const fetchRates = async () => {
    setIsLoading(true);
    const { data, error } = await apiClient.get<Rate[]>(
  ...
  const handleAddRate = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    const { data, error } = await apiClient.post<Rate>(
      `/properties/${propertyId}/rates`,
      newRateForm
    );
    ...
    toast({
      title: "Rate Added!",
      description: `New rate effective from ${data?.effectiveFrom}.`,
    });
    ...
    <div className="rounded-xl border border-border bg-surface overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 border-b border-border">
        ...
        {isOwner && (
          <TableActionDialog
            ...
            isSubmitDisabled={
              !newRateForm.effectiveFrom || newRateForm.consumptionRate <= 0
            }
          >
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewRateForm({
                    ...newRateForm,
                    exportRate: parseFloat(e.target.value) || 0,
                  })
                }
              />
            </div>
      ...
      {isLoading ? (
        ...
        <div className="px-6 py-10 text-center text-muted-foreground text-sm border-dashed border border-border rounded-lg m-6">
          ...
          {isOwner && (
            <p className="mt-1 text-xs">Add the first rate version above.</p>
          )}
        </div>
      ) : (
              ...
              {rates.map((rate, ri) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right font-numbers">
                    ₹{rate.consumptionRate.toFixed(3)}
                  </td>
                  <td className="px-4 py-3 text-right font-numbers">
                    ₹{rate.exportRate.toFixed(3)}
                  </td>
