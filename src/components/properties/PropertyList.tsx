...
export function PropertyList() {
  ...
  useEffect(() => {
    async function fetchProperties() {
      const res = await apiClient.get<PropertiesResponse>(
  ...
  const handleUnarchive = async (propertyId: string) => {
    setLoading(true);
    const res = await apiClient.patch<{ success: boolean; data: Property }>(
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-4">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="rounded-xl border border-border bg-surface p-6 min-h-[140px] flex flex-col justify-center"
          >
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            href="/properties/new"
            className="btn btn-primary no-underline text-sm font-semibold whitespace-nowrap"
          >
            + Add Property
          </a>
        </div>
        ...
        {properties.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 bg-surface border border-border rounded-xl text-center w-full max-w-2xl mx-auto my-4">
            ...
            <p className="text-muted-foreground mb-8 max-w-sm font-body">
              Meterly tracks electricity bills between you and your tenants. Add
              a property to get started.
            </p>
            <a
              href="/properties/new"
          ...
          <>
            {filteredProperties.length === 0 ? (
              ...
              filteredProperties.map((prop) => {
                ...
                if (
                  prop.lastBillMonth &&
                  prop.lastBillTenantCount !== null &&
                  prop.lastBillPaidCount !== null
                ) {
                  const allPaid =
                    prop.lastBillPaidCount === prop.lastBillTenantCount &&
                    prop.lastBillTenantCount > 0;
                  const nonePaid =
                    prop.lastBillPaidCount === 0 &&
                    prop.lastBillTenantCount > 0;
                  const colorClass = allPaid
                    ? "text-emerald-500"
                    : nonePaid
                      ? "text-red-500"
                      : "text-amber-500";
                  ...
                  paymentIndicator = (
                    ...
                    >
                      {icon}
                      {prop.lastBillPaidCount} of {prop.lastBillTenantCount}{" "}
                      paid
                    </span>
                  );
                }
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-6">
                      {!prop.soloMode && (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span>
                            {prop.tenantCount || 0} active tenant
                            {prop.tenantCount !== 1 ? "s" : ""}
                          </span>
                        </div>
                      )}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-end justify-between pt-4 border-t border-border gap-4">
                        ...
                        {prop.lastBillMonth ? (
                          <p className="text-sm text-muted-foreground m-0">
                            ...
                            <span className="font-numbers font-medium text-foreground">
                              {formatCurrency(prop.lastBillTotal || 0)}
                            </span>
                            {!prop.soloMode && (
                              <>
                                <span className="mx-2">•</span>
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        <a
                          ...
                          className="btn btn-primary py-1.5 px-3 text-xs no-underline font-medium"
                        >
                          View Dashboard
                        </a>
                      </div>
