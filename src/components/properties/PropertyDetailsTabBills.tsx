...
export function PropertyDetailsTabBills({
  ...
  // Reusable states and hook for reopen dialog.
  const { toast } = useToast();
  const [periodToReopen, setPeriodToReopen] = React.useState<{
    id: string;
    month: string;
  } | null>(null);
  ...
  const getMarkerPosition = (): string | null => {
    ...
    const firstBefore = flattenedBills.find(
      (b) => new Date(b.periodMonth) < changeDate
    );
        ...
        {billsData && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="rounded-xl border border-border bg-surface p-5">
              ...
              <div className="text-2xl font-bold font-numbers text-foreground">
                {formatCurrency(billsData.summary.totalBilled)}
              </div>
            </div>
            <div className="rounded-xl border border-border bg-surface p-5">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-emerald-600 mb-1">
                Collected
              </h3>
              <div className="text-2xl font-bold font-numbers text-emerald-600">
                {formatCurrency(billsData.summary.totalCollected)}
              </div>
            </div>
            <div className="rounded-xl border border-border bg-surface p-5">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-rose-600 mb-1">
                Outstanding
              </h3>
              <div className="text-2xl font-bold font-numbers text-rose-600">
                {formatCurrency(billsData.summary.totalOutstanding)}
              </div>
            </div>
          </div>
        )}
          ...
          {isLoadingBills ? (
            ...
            <div className="p-6">
              <EmptyState
                ...
                description="No bills match the selected filters."
              />
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-border">
                  {flattenedBills.map((b, ri) => {
                    const isBeforeChange =
                      property.soloModeChangedAt &&
                      new Date(b.periodMonth) <
                        new Date(property.soloModeChangedAt);
                    const showMarker =
                      isBeforeChange &&
                      !markerRendered &&
                      b.billId === markerBillId;
                      ...
                      <React.Fragment key={b.billId}>
                        {showMarker && (
                          <tr>
                            <td
                              colSpan={6}
                              className="px-4 py-2.5 bg-surface-raised/30 text-center border-y border-dashed border-border"
                            >
                              <span className="inline-flex items-center gap-2 text-xs font-medium text-muted-foreground">
                                <span className="h-px w-8 bg-border inline-block" />
                                Tracking mode switched to{" "}
                                {property.soloMode ? "Solo" : "Landlord"} on{" "}
                                {new Date(
                                  property.soloModeChangedAt!
                        ...
                        >
                          <td className="px-4 py-3 font-medium">
                            {new Date(b.periodMonth).toLocaleString("default", {
                              month: "long",
                              year: "numeric",
                            })}
                          </td>
                          <td className="px-4 py-3 text-muted-foreground">
                            {b.tenantName || "Unknown"}
                          </td>
                          <td className="px-4 py-3 text-right font-numbers">
                            {b.totalConsumption
                              ? (
                                  b.totalConsumption *
                                  (b.splitPercentage / 100)
                                ).toFixed(2)
                              : "0.00"}{" "}
                            kWh
                          </td>
                          <td className="px-4 py-3 text-right font-numbers font-medium">
                            {formatCurrency(b.totalDue)}
                          </td>
                          <td className="px-4 py-3">
                            <Badge
                              variant={
                                b.status === "paid" ? "success" : "warning"
                              }
                            >
                              {b.status}
                            </Badge>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to reopen the billing period for{" "}
              {periodToReopen &&
                new Date(periodToReopen.month + "T00:00:00Z").toLocaleString(
                  "default",
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch(
                    `/api/periods/${periodToReopen.id}/reopen`,
                    { method: "PATCH" }
                  );
                  ...
                  if (json.success) {
                    ...
                    toast({
                      ...
                      title: "Error",
                      description:
                        json.error?.message || "Failed to reopen period.",
                    });
                  }
                } catch {
                  toast({
