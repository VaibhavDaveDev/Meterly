...
export function EditRequestsPage({ propertyId }: EditRequestsPageProps) {
  ...
  const [requestToApprove, setRequestToApprove] = useState<EditRequest | null>(
    null
  );
  ...
  const fetchRequests = async () => {
    try {
      ...
      const json = (await res.json()) as {
        success: boolean;
        data: { pending: EditRequest[]; resolved: EditRequest[] };
      ...
      if (json.success) {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: json.error?.message || "Failed to load requests",
        });
      }
    } catch (error) {
      console.error(error);
  ...
  const confirmApprove = async () => {
    ...
    try {
      ...
      if (json.success) {
        toast({
          title: "Success",
          description: "Request approved and recalculation queued",
        });
        fetchRequests();
      } else {
        toast({
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `/api/edit-requests/${selectedRequest.id}/review`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Manage tenant-requested corrections
          </p>
        </div>
        {pending.length > 0 && (
          <Badge
      ...
      <div className="space-y-4">
        {pending.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center space-y-3 p-6">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Tenants can request corrections from their bill page.
                </p>
              </div>
            </div>
          </div>
          ...
          pending.map((request) => (
            <div
              key={request.id}
              className="border border-border rounded-xl bg-card text-card-foreground shadow-sm"
            >
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {request.periodStatus === "confirmed" && (
                        <Badge
                          variant="warning"
                          className="text-amber-600 border-amber-600/30 bg-amber-50 dark:bg-amber-950/20 text-xs py-0.5 px-2"
                        >
                          Confirmed Period
                        </Badge>
                      )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-card border rounded-md p-4 space-y-2">
                    ...
                    <div className="mt-4 pt-4 border-t flex justify-between items-center text-sm">
                      ...
                      <Badge
                        variant={
                          request.impactSummary.unitsDelta > 0
                            ? "warning"
                            : "success"
                        }
                      >
                        {request.impactSummary.unitsDelta > 0 ? "+" : ""}
                        {request.impactSummary.unitsDelta} units
                      </Badge>
                    </div>
                  </div>
                </div>
          ...
          {isResolvedExpanded && (
            <div className="mt-4 space-y-4">
              {resolved.map((request) => (
                <div
                  key={request.id}
                  className="border border-border rounded-xl bg-card text-card-foreground shadow-sm opacity-75 hover:opacity-100 transition-opacity"
                >
      ...
      <Dialog open={isRejectModalOpen} onOpenChange={setIsRejectModalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Please provide a reason for rejecting this edit. The tenant will
              be notified.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setRequestToApprove(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Approve Edit Request</DialogTitle>
            <DialogDescription>
              Are you sure you want to approve this edit for{" "}
              {requestToApprove &&
                new Date(
                  requestToApprove.periodMonth + "T00:00:00Z"
                ).toLocaleString("default", {
                  month: "long",
                  year: "numeric",
                  timeZone: "UTC",
                })}
              ? The bill will be recalculated and the tenant will be notified.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
