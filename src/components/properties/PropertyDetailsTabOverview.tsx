...
export function PropertyDetailsTabOverview({
  state,
}: {
  ...
  const handleConfirmPeriod = async () => {
    ...
    try {
      const res = await fetch(`/api/periods/${activePeriod.id}/confirm`, {
        method: "PATCH",
      });
      const data = (await res.json()) as { error?: { message: string } };
      if (!res.ok)
        throw new Error(data.error?.message || "Failed to confirm period");
  ...
  const handleReopenPeriod = async () => {
    ...
    setIsReopening(true);
    try {
      const res = await fetch(`/api/periods/${activePeriod.id}/reopen`, {
        method: "PATCH",
      });
      const data = (await res.json()) as { error?: { message: string } };
      if (!res.ok)
        throw new Error(data.error?.message || "Failed to reopen period");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-surface p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          {activePeriod ? (
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-sm text-muted-foreground">
                {new Date(activePeriod.periodMonth).toLocaleString("default", {
                  month: "long",
                  year: "numeric",
                })}
              </span>
              ...
              <Badge
                variant={
                  activePeriod.calculationMode === "solar" ? "success" : "info"
                }
              >
                {activePeriod.calculationMode === "solar"
                  ? "Solar"
                  : "Grid Only"}
              </Badge>
            </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {isOwner && (
            <div className="flex gap-2">
                ...
                <DialogContent className="sm:max-w-[425px]">
                  <form onSubmit={handleStartPeriod}>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Select the month to start tracking. You can backfill
                        past months too — useful if you're catching up on older
                        data.
                      </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter>
                      ...
                      <Button
                        type="submit"
                        disabled={isStartingPeriod || !periodMonth}
                      >
              ...
              <Button variant="outline" size="sm" asChild>
                <a
                  href={`/properties/${localProperty.id}/periods/new?past=true`}
                >
                  Add Past Data
                </a>
              </Button>
          ...
          {activePeriod && activePeriod.status !== "pending_approval" && (
            <Button
              size="sm"
              variant={activePeriod.status === "draft" ? "default" : "outline"}
              asChild
            >
              <a
                href={`/properties/${localProperty.id}/periods/${activePeriod.id}`}
              >
      ...
      {isOwner && pendingEditRequestCount > 0 && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-amber-900 dark:text-amber-100">
                {pendingEditRequestCount === 1
                  ? "1 pending correction request"
                  : `${pendingEditRequestCount} pending correction requests`}
              </p>
              <p className="text-sm text-amber-800/80 dark:text-amber-200/70">
                {pendingEditRequestCount === 1
                  ? "A tenant has submitted a meter reading correction request."
                  : "Tenants have submitted meter reading correction requests."}
              </p>
            </div>
          </div>
          <a
            href={`/properties/${localProperty.id}/edit-requests`}
            className="shrink-0 inline-flex items-center gap-1.5 text-sm font-medium text-amber-700 dark:text-amber-300 underline-offset-2 hover:underline"
          >
            Review requests →
          </a>
        </div>
      ...
      {activePeriod?.status === "draft" && activePeriod.bills?.length === 0 && (
        <div className="rounded-xl border-2 border-dashed border-accent/30 bg-accent/5 p-6 text-center mt-6">
          <p className="font-semibold text-foreground mb-2">
            Ready to add this month's readings?
          </p>
        ...
        (activePeriod.bills?.length ?? 0) > 0 && (
          <div className="rounded-xl border border-warning bg-warning/10 p-4 text-warning-foreground mt-6 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm">
                This billing period is open for corrections. Bills will be
                recalculated and notifications resent when confirmed.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="rounded-xl border border-border bg-surface overflow-hidden">
          <div className="px-5 py-4 border-b border-border bg-muted/20">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
              {isOwner
                ? `Current Period Bills (${new Date(activePeriod.periodMonth).toLocaleString("default", { month: "long", year: "numeric" })})`
                : "Your Current Bill"}
            </h3>
          </div>
          <div className="divide-y divide-border">
            {activePeriod.bills
              .filter((b) => isOwner || b.isSelf)
              .map((bill) => (
                <div
                  key={bill.billId}
                ...
                >
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xl font-numbers font-bold text-foreground">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-border bg-surface p-5">
          ...
          <Badge
            variant={property.hasSolar ? "success" : "muted"}
      ...
      <PropertyGraphsSection chartData={chartData} isLoading={isLoadingChart} />
      {/* ── Rates Table ── */}
      <div className="pt-4">
