...
export function RequestsManager() {
  ...
  const handleReview = async (action: "approve" | "reject") => {
    ...
    const { error } = await apiClient.patch(
      `/edit-requests/${selectedRequest.id}/review`,
      {
        action,
    ...
    toast({
      ...
      description:
        action === "approve"
          ? "Bills have been scheduled for recalculation."
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Review meter reading correction requests submitted by your tenants
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center border border-dashed rounded-xl bg-surface/50 text-muted-foreground">
          ...
          <p className="text-sm max-w-sm">
            When tenants request changes to their bills, you'll see them here to
            review and approve.
          </p>
        </div>
      ) : (
        <div className="grid gap-4">
          {requests.map((request) => (
            <div
              key={request.id}
              className="p-5 border border-border rounded-xl bg-surface flex flex-col md:flex-row md:items-center justify-between shadow-sm gap-4"
            >
              <div className="space-y-1">
                ...
                <div className="font-medium text-foreground flex items-center gap-2">
                  ...
                  {new Date(request.periodMonth).toLocaleString("default", {
                    month: "long",
                    year: "numeric",
                  })}
      ...
      {resolvedCount > 0 && (
        <div className="pt-6 text-center">
          <p className="text-sm text-muted-foreground">
            {resolvedCount} resolved request{resolvedCount === 1 ? "" : "s"}{" "}
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Review the tenant's request for {selectedRequest?.propertyName} (
              {selectedRequest
                ? new Date(selectedRequest.periodMonth).toLocaleString(
                    "default",
              ...
              selectedRequest?.currentValues && (
                <div className="space-y-2 pt-4 border-t border-border">
                  ...
                  <div className="bg-muted/50 rounded-md p-3 text-sm space-y-2">
                    {["importEnd", "exportEnd", "solarGenerationEnd"].map(
                      (key) => {
