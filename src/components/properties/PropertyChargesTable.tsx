...
export function PropertyChargesTable({
  ...
  const fetchCharges = async () => {
    setIsLoading(true);
    const { data } = await apiClient.get<CustomCharge[]>(
  ...
  const handleSaveCharge = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    if (editingChargeId) {
      const { data, error } = await apiClient.patch<CustomCharge>(
        `/properties/charges/${editingChargeId}`,
        chargeForm
      );
  ...
  const toggleChargeStatus = async (charge: CustomCharge) => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: `${charge.name} is now ${!charge.isActive ? "active" : "inactive"}.`,
      });
    }
  };
  ...
  const confirmDeleteCharge = async () => {
    ...
    const { error } = await apiClient.delete(
      `/properties/charges/${chargeToDelete.id}`
    );
    if (error) {
      toast({
        variant: "destructive",
      ...
      toast({
        ...
        description: `${chargeToDelete.name} has been removed.`,
      });
    }
