...
export function SubmitReadingPage({ periodId }: SubmitReadingPageProps) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          The period may not exist or you may not have access.
        </p>
  ...
  const canOverrideRates =
    isOwner && (period.status === "submitted" || period.status === "confirmed");
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
      const photosRes = await fetch(
        `/api/uploads/bill-photos?periodId=${periodId}`
      );
      ...
      const hasPhotos =
        photosData.success && photosData.data && photosData.data.length > 0;
    ...
    try {
      ...
      if (shouldSaveReadings) {
        ...
        const payload = {
          ...
          allowRollover,
          oneOffCharges:
            isOwner && oneOffCharges.length > 0 ? oneOffCharges : undefined,
        };
      ...
      if (editRates && canOverrideRates) {
        ...
        if (!ratesJson.success) {
          toast({
            ...
            description: ratesJson.error?.message,
          });
          setIsSubmitting(false);
          return;
        }
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <Badge variant={isSolar ? "success" : "muted"}>
                  {isSolar ? "Solar" : "Grid-only"}
                </Badge>
              </div>
            </div>
          </div>
        ...
        {existingReading && canEdit && (
          <div className="p-4 rounded-lg border border-amber-500/30 bg-amber-500/10 text-amber-500 text-sm">
            <strong>Editing Mode:</strong> Editing this reading will recalculate
            bills for all tenants and notify them. A reason is required.
          </div>
        )}
          ...
          <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Edit readings below, then go back to the property overview and
                confirm the period to lock it again.
              </p>
            </div>
          </div>
        )}
        ...
        {period.status === "pending_approval" && isOwner && (
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-foreground">
                Tenant Reading Pending Approval
              </p>
              <p className="text-xs text-muted-foreground">
                A tenant has submitted readings for this period. Go to{" "}
                <a
                  href={`/properties/${property.id}/edit-requests`}
          ...
          <div className="space-y-8">
            ...
            {canRequestEdit && tenancyId && (
              <div className="rounded-xl border border-accent/20 bg-accent/5 p-5 text-center">
                <p className="text-sm font-medium text-foreground mb-2">
                  See an error in your readings?
                </p>
                <p className="text-xs text-muted-foreground mb-3">
                  The billing period is confirmed. You can submit a correction
                  request to your owner.
                </p>
                <Button asChild size="sm">
                  <a
                    href={`/tenancies/${tenancyId}/edit-requests/new?periodId=${period.id}`}
                  >
            ...
            <div className="rounded-xl border border-border bg-surface p-6 space-y-6">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Enter the numbers from your meters right now.
                </p>
              </div>
              ...
              {hasOcrData && (canSubmit || canEdit) && (
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-amber-600 dark:text-amber-400">
                  <strong>Verify before submitting.</strong> These figures were
                  extracted automatically. OCR can make mistakes — always check
                  against your physical meter or bill before submitting.
                </div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                {isSolar && (
                  <>
                    <MeterInput
                      ...
                      value={
                        solarGenerationEnd ||
                        (existingReading?.solarGenerationEnd.toString() ?? "")
                      }
                    ...
                    <MeterInput
                      ...
                      value={
                        exportEnd ||
                        (existingReading?.exportEnd.toString() ?? "")
                      }
                ...
                <div className="space-y-3">
                  <MeterInput
                    ...
                    value={
                      importEnd || (existingReading?.importEnd.toString() ?? "")
                    }
                  ...
                  {(canSubmit || canEdit) &&
                    (allowRollover ||
                      validationErrors.some((e) =>
                        e.includes("lower than the previous reading")
                      )) && (
                      <div className="space-y-1 pl-1">
                        ...
                        {allowRollover && (
                          <p className="text-xs text-amber-600 dark:text-amber-500 font-medium">
                            Only check this if your meter actually reached its
                            maximum and rolled back to zero. If you made a
                            mistake in the previous reading, ask your landlord
                  ...
                  <>
                    ...
                    {hasWarning && (
                      <label className="flex items-start gap-2 p-3 rounded-lg border border-amber-500/30 bg-amber-500/10">
                        ...
                        <span className="text-sm text-amber-600 dark:text-amber-500">
                          I acknowledge that some readings have unusually high
                          increases or decimals and I have double-checked them.
                        </span>
                      </label>
                    )}

                    {((existingReading && canEdit) ||
                      (editRates && canOverrideRates)) && (
                      <div className="space-y-2">
                        ...
                        <textarea
                          ...
                          value={reason}
                          onChange={(e) =>
                            setReason(e.target.value.slice(0, 1000))
                          }
                          required
                          minLength={10}
                          maxLength={1000}
                    ...
                    <Button
                      ...
                      disabled={
                        !(canSave || editRates) ||
                        isSubmitting ||
                        (editRates && reason.length < 10)
                      }
                    >
                      {isSubmitting
                        ? "Submitting..."
                        : property.readingsRequireApproval && !isOwner
