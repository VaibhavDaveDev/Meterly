  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/properties/${propertyId}`).then(
        (r) => r.json() as Promise<{ success: boolean; data: PropertyInfo }>
      ),
      fetch(`/api/properties/${propertyId}/periods?limit=1`).then(
        (r) => r.json() as Promise<{ success: boolean; data: PeriodInfo[] }>
      ),
    ])
      .then(([propData, periodsData]) => {
        ...
        if (periodsData.success && periodsData.data.length > 0) {
          ...
          if (lastP.reading) {
            ...
            setSolarGenStart(
              lastP.reading.solarGenerationEnd?.toString() || "0"
            );
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    if (isNaN(impS) || isNaN(impE)) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Import readings are required.",
      });
      return;
    }
    ...
    if (impE < impS) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Import end reading cannot be less than start reading.",
      });
      return;
    }
    ...
    if (property?.hasSolar) {
      ...
      if (solE < solS) {
        toast({
          ...
          title: "Error",
          description:
            "Solar Generation end reading cannot be less than start reading.",
        });
        return;
      }
      if (expE < expS) {
        toast({
          ...
          title: "Error",
          description:
            "Grid Export end reading cannot be less than start reading.",
        });
        return;
      }
    ...
    try {
      ...
      const res = await fetch(`/api/properties/${propertyId}/periods`, {
        ...
        body: JSON.stringify({
          ...
          readings: {
            importStart: impS,
            importEnd: impE,
            solarGenerationStart: property?.hasSolar
              ? parseFloat(solarGenStart || "0")
              : 0,
            solarGenerationEnd: property?.hasSolar
              ? parseFloat(solarGenEnd || "0")
              : 0,
            exportStart: property?.hasSolar
              ? parseFloat(exportStart || "0")
              : 0,
      ...
      if (json.success) {
        toast({
          title: "Success",
          description: "Historical period and readings created successfully.",
        });
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: json.error?.message || "Failed to create period",
        });
      }
    } catch (err) {
      console.error(err);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground m-0 mt-1">
              Enter the exact meter readings for this past month. Ensure
              readings follow chronological order.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl border border-border bg-surface p-6 space-y-6">
        ...
        {property?.hasSolar && (
          <div className="space-y-6 border-t border-border pt-6">
            <h4 className="text-md font-semibold text-foreground m-0">
              Solar Readings
            </h4>
            <div className="grid gap-6 sm:grid-cols-2">
              <MeterInput
      ...
      <div className="rounded-xl border border-border bg-surface p-6 space-y-4">
        ...
        <p className="text-sm text-muted-foreground m-0">
          Add any one-off adjustments for this period (e.g. late fees,
          discounts).
        </p>
            ...
            >
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">
                  {c.chargedToTenant ? "Charged to Tenant" : "Paid by Owner"}
                </span>
              </div>
