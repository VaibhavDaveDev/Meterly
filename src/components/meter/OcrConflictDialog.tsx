...
export function OcrConflictDialog({
  proposals,
  onResolve,
}: OcrConflictDialogProps) {
  const [resolutions, setResolutions] = useState<
    Record<string, "keep_mine" | "use_ocr">
  >(Object.fromEntries(proposals.map((p) => [p.field, "keep_mine"])));

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-xl border border-border bg-surface p-6 shadow-xl">
        <h2 className="text-base font-semibold font-heading mb-1">
          OCR found different figures
        </h2>
        ...
        <p className="text-xs text-amber-600/80 mb-5">
          Note: OCR can misread digits, especially on LCD meter displays. Common
          mistakes: 3/8, 0/8, 1/7. Always check against the physical meter.
        </p>
            ...
            <div key={p.field} className="rounded-lg border border-border p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() =>
                    setResolutions((r) => ({ ...r, [p.field]: "keep_mine" }))
                  }
                ...
                >
                  <span className="block font-numbers font-bold text-base">
                    {p.currentValue}
                  </span>
                ...
                <button
                  type="button"
                  onClick={() =>
                    setResolutions((r) => ({ ...r, [p.field]: "use_ocr" }))
                  }
                  className={`rounded-lg border p-3 text-sm transition-colors ${
