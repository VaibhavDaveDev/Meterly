...
import {
  calculateSolarBill,
  calculateGridOnlyBill,
} from "../../api/lib/billing-engine";
...
export function LiveCalculationPreview({
  ...
  const solarEnd =
    currentValues.solarGenerationEnd ?? startValues.solarGenerationStart;
  ...
  const renderDeltaRow = (
    ...
    <div className="flex items-center justify-between py-1 text-sm">
      ...
      <span
        className={`w-1/4 text-right font-mono text-xs ${delta > 0 ? "text-emerald-500" : "text-muted-foreground"}`}
      >
          ...
          {isSolar && (
            <>
              {renderDeltaRow(
                ...
                currentValues.solarGenerationEnd,
                calculation.solarGenerated
              )}
              {renderDeltaRow(
                "Export",
                startValues.exportStart,
                currentValues.exportEnd,
                calculation.gridExported
              )}
            </>
          )}
          {renderDeltaRow(
            ...
            currentValues.importEnd,
            calculation.gridImported
          )}
        </div>
          ...
          <div className="text-sm space-y-3 font-mono">
              ...
              <>
                <div>
                  ...
                  <div className="text-muted-foreground text-xs ml-6">
                    Solar End - Solar Start ={" "}
                    {calculation.solarGenerated.toFixed(2)} units
                  </div>
                </div>
                <div>
                  ...
                  <div className="text-muted-foreground text-xs ml-6">
                    ...
                    {calculation.gridExported.toFixed(2)} units
                  </div>
                </div>
                <div>
                  <div className="text-foreground">(3) Solar Self-Consumed</div>
                  <div className="text-muted-foreground text-xs ml-6">
                    Generated ({calculation.solarGenerated.toFixed(2)}) -
                    Exported ({calculation.gridExported.toFixed(2)}) ={" "}
                    {calculation.solarSelfConsumed.toFixed(2)} units
                  </div>
                </div>
              </>
            )}
            ...
            <div>
              <div className="text-foreground">
                ({isSolar ? "4" : "1"}) Imported from Grid
              </div>
              <div className="text-muted-foreground text-xs ml-6">
                Import End - Import Start ={" "}
                {calculation.gridImported.toFixed(2)} units
              </div>
            </div>
            ...
            <div className="pt-2">
              ...
              <div className="text-muted-foreground text-xs ml-6">
                {calculation.totalConsumption.toFixed(2)} × {splitPercentage}% ={" "}
                {calculation.tenantConsumption.toFixed(2)} units
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 pt-4 border-t border-border">
          ...
          <div className="space-y-2 text-sm font-mono">
            <div className="flex justify-between items-center">
              <span>
                Consumption ({calculation.tenantConsumption.toFixed(2)} u × ₹
          ...
          {isSolar && (
            <div className="p-3 bg-muted/10 rounded-md border border-border text-xs text-muted-foreground space-y-1">
              ...
              <p className="font-mono">
                {calculation.gridExported.toFixed(2)} × ₹{rates.exportRate} ={" "}
                <span className="text-emerald-500 font-semibold">
                  {formatCurrency(calculation.exportRefund || 0)}
                </span>
              </p>
            </div>
          )}
