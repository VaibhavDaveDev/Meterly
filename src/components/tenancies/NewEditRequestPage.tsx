...
export function NewEditRequestPage({
  ...
  const [pendingRequest, setPendingRequest] = useState<EditRequest | null>(
    null
  );
    ...
    async function loadConfirmedPeriods() {
      ...
      try {
        const res = await fetch(
          `/api/tenancies/${tenancyId}/confirmed-periods`
        );
        if (res.status === 401) {
        ...
        if (existing) {
          ...
          setImportEnd(
            pv.importEnd !== undefined
              ? String(pv.importEnd)
              : String(pData.data.existingReading?.importEnd || "")
          );
          setExportEnd(
            pv.exportEnd !== undefined
              ? String(pv.exportEnd)
              : String(pData.data.existingReading?.exportEnd || "")
          );
          setSolarGenerationEnd(
            pv.solarGenerationEnd !== undefined
              ? String(pv.solarGenerationEnd)
              : String(pData.data.existingReading?.solarGenerationEnd || "")
          );
  ...
  const getErrors = () => {
    ...
    if (
      importEnd !== "" &&
      !isNaN(impVal) &&
      impVal < startValues.importStart
    ) {
    ...
    if (isSolar) {
      ...
      if (
        exportEnd !== "" &&
        !isNaN(expVal) &&
        expVal < startValues.exportStart
      ) {
      ...
      if (
        solarGenerationEnd !== "" &&
        !isNaN(solVal) &&
        solVal < startValues.solarGenerationStart
      ) {
    ...
    const solValNum =
      solarGenerationEnd !== "" ? Number(solarGenerationEnd) : currentSolar;
    ...
    const allFilled =
      importEnd !== "" &&
      (isSolar ? exportEnd !== "" && solarGenerationEnd !== "" : true);
  ...
  if (isLoadingPeriods || (selectedPeriodId && isLoading)) {
    return (
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-sm text-muted-foreground max-w-sm mx-auto">
          We encountered an error while fetching your confirmed billing periods.
  ...
  if (!isLoadingPeriods && confirmedPeriods.length === 0) {
    return (
      <div className="text-center py-12">
        <FileText className="w-12 h-12 mx-auto text-muted-foreground/30 mb-4" />
        <p className="text-lg font-medium mb-1">
          No bills to request edits for
        </p>
        <p className="text-sm text-muted-foreground max-w-sm mx-auto">
          Your landlord hasn&apos;t confirmed any billing periods yet. Once a
          period is confirmed, you&apos;ll be able to request corrections here.
        </p>
        <Button
          variant="outline"
          className="mt-4"
    ...
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">
          No confirmed readings found for the selected period.
        </p>
        <Button
          variant="outline"
          className="mt-4"
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold font-heading text-foreground m-0">
              Request Edit
            </h1>
            <p className="text-sm text-muted-foreground m-0">
              {property.name} •{" "}
              {new Date(period.periodMonth).toLocaleString("default", {
                month: "long",
                year: "numeric",
              })}
            </p>
        ...
        <div className="rounded-xl border border-blue-500/30 bg-blue-500/10 p-6 text-center max-w-lg mx-auto mt-8">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Correction requests are only needed for billing periods your
            landlord has already confirmed. This period is still open, so you
            can edit the readings directly.
          </p>
          <Button asChild>
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    if (hasErrors) {
      toast({
        ...
        description: "Please resolve all highlighted errors before submitting.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
        ...
        proposedValues: {
          ...
          ...(isSolar
            ? {
                ...
                solarGenerationEnd: solarGenerationEnd
                  ? Number(solarGenerationEnd)
                  : undefined,
              }
            : {}),
        },
      ...
      const resData = (await res.json()) as {
        ...
        data?: { overwrote?: boolean };
      };
      if (!res.ok)
        throw new Error(resData.error?.message || "Failed to submit request");

      if (resData.data?.overwrote) {
        toast({
          title: "Request Updated",
          description: "Your previous pending request was overwritten.",
        });
      } else {
        toast({
          title: "Request Submitted",
          description: "The property owner will review your proposed changes.",
        });
      }
    ...
    <div className="space-y-6">
        ...
        >
          ...
          {confirmedPeriods.map((p) => (
            <option key={p.id} value={p.id}>
              {new Date(p.periodMonth).toLocaleString("default", {
                month: "long",
                year: "numeric",
              })}
        ...
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground m-0 mt-1">
              This billing period has been locked by your landlord. Your
              correction request will be reviewed. If approved, your bill will
              be recalculated.
            </p>
          </div>
        </div>
      ...
      {pendingRequest && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-5 flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-semibold text-amber-600 dark:text-amber-400 m-0">
              Pending Request Exists
            </h4>
            <p className="text-sm text-amber-600/80 dark:text-amber-400/80 m-0 mt-1">
              You already have a pending edit request for this billing period.
              Submitting this form will overwrite your previous request.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="rounded-xl border border-border bg-surface p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center text-xs text-muted-foreground">
              <span>
                The property owner will see this reason when reviewing your
                request.
              </span>
