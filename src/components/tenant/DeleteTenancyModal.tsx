...
interface DeleteTenancyModalProps {
  isOpen: boolean;
  propertyName: string;
  allPaid: boolean;
  onConfirm: () => void;
...
export function DeleteTenancyModal({
  isOpen,
  propertyName,
  allPaid,
  onConfirm,
  onCancel,
}: DeleteTenancyModalProps) {
  ...
  const isConfirmDisabled =
    (needsUnpaidCheck && !unpaidChecked) || !deletionChecked;
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-6 border border-border bg-surface p-6 shadow-xl rounded-xl animate-in fade-in zoom-in-95 font-body">
            ...
            <Dialog.Description asChild>
              <div className="text-sm text-muted-foreground space-y-4">
                <p className="m-0">
                  ...
                  and all your associated bills and meter readings from your
                  account history.{" "}
                ...
                {needsUnpaidCheck && (
                  <p className="m-0 text-amber-600 dark:text-amber-400">
                    You have unpaid bill(s) at {propertyName}. Deleting this
                    record from your account does not cancel your bills or
                    forgive any outstanding balance.
                  </p>
                )}
              </div>
            </Dialog.Description>
          ...
          <div className="flex flex-col gap-3 py-2">
            {needsUnpaidCheck && (
              <div className="flex items-center space-x-3 bg-amber-500/10 p-3 rounded-lg border border-amber-500/20">
                <Checkbox
                  id="unpaid-check"
                ...
                >
                  I understand my outstanding bills are not cancelled
                </label>
              </div>
            )}
            ...
            <div className="flex items-center space-x-3 bg-red-500/10 p-3 rounded-lg border border-red-500/20">
              ...
              >
                I understand this removes the property and its history from my
                account
              </label>
            </div>
          </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 gap-2 sm:gap-0">
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-md text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring px-4 py-2 shadow-sm bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 disabled:bg-red-600"
            >
              Remove property
            </button>
          </div>
