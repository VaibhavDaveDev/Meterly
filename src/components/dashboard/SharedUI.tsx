...
export function KpiCard({
  ...
  return (
    <div
      className={`rounded-xl border p-5 flex flex-col justify-center ${bgClass}`}
    >
      <div className="flex items-center gap-0.5 mb-1">
        <span className="text-xs uppercase tracking-wide text-muted-foreground font-semibold font-heading">
          {label}
        </span>
        {tooltip && <TooltipIcon content={tooltip} />}
      </div>
      <span className={`text-3xl font-bold font-numbers ${valueColorClass}`}>
        {value}
      </span>
      {subtext && (
      ...
      {delta !== undefined && (
        <span
          className={`text-xs mt-1 font-numbers ${
            delta.value > 0
              ? "text-amber-500"
              : delta.value < 0
                ? "text-emerald-500"
...
export function ChartCard<T>({
  ...
  const slicedData = data ? (range === "all" ? data : data.slice(-range)) : [];
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4 gap-2">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground font-heading m-0">
          {title}
        </h3>
      ...
      >
        <div
          className={`chart-area w-full ${isFullscreen ? "flex-1" : ""}`}
          style={{ height: isFullscreen ? undefined : 240, minHeight: 240 }}
        >
...
export function SectionHeading({
  ...
  return (
    <h2
      id={id}
      className="mb-3 text-[0.8125rem] font-semibold uppercase tracking-wider text-muted-foreground font-heading"
    >
      {children}
    </h2>
  );
        ...
        <table className="w-full text-sm text-left">
          <thead>
            <tr className="border-b border-border">
              <th className="pb-2 font-semibold text-muted-foreground uppercase tracking-wider text-[11px]">
                Metric
              </th>
              <th className="pb-2 font-semibold text-muted-foreground uppercase tracking-wider text-[11px]">
                Last Month
          ...
          <tbody className="divide-y divide-border">
            {rows.map((row, i) => {
              const change =
                row.lastMonthValue > 0
                  ? ((row.thisMonthValue - row.lastMonthValue) /
                      row.lastMonthValue) *
                    100
                  : null;
              ...
              const isGood =
                change !== null && (row.invertColors ? change < 0 : change > 0);

              const fmt = (val: number) =>
                row.format === "currency" ? formatCurrency(val) : `${val} kWh`;
              ...
              return (
                <tr key={i}>
                  ...
                  <td className="py-3 font-numbers">
                    {fmt(row.lastMonthValue)}
                  </td>
                  <td
                    className={`py-3 font-numbers ${isGood ? "text-emerald-500 font-semibold" : ""}`}
                  >
                    {fmt(row.thisMonthValue)}
                  </td>
