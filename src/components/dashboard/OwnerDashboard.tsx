...
export function OwnerDashboard({
  ...
  return (
    <section className="space-y-6">
        ...
        stats.totalInvitedTenants === 0 && (
          <div className="bg-surface border border-accent bg-accent/5 rounded-xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-[0.9375rem] m-0">
                Send an invite link to your tenant to start tracking their
                bills.
              </p>
            </div>
            <a
              href="/properties"
              className="btn btn-primary whitespace-nowrap text-sm font-semibold"
            >
              Go to Properties
            </a>
          </div>
      ...
      {stats.totalProperties > 0 &&
        (stats.activeTenants > 0 || stats.totalInvitedTenants > 0) &&
        stats.totalPeriods === 0 && (
          <div className="bg-surface border border-accent bg-accent/5 rounded-xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-[0.9375rem] m-0">
                When the month ends, add meter readings to generate your first
                bill.
              </p>
            </div>
            <a
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          ...
          delta={
            stats.momComparison && stats.momComparison.lastMonth.billed > 0
              ? {
                  value:
                    ((stats.momComparison.thisMonth.billed -
                      stats.momComparison.lastMonth.billed) /
                      stats.momComparison.lastMonth.billed) *
                    100,
                }
              : undefined
        ...
        <KpiCard
          ...
          delta={
            stats.momComparison &&
            stats.momComparison.lastMonth.solarEarnings > 0
              ? {
                  value:
                    ((stats.momComparison.thisMonth.solarEarnings -
                      stats.momComparison.lastMonth.solarEarnings) /
                      stats.momComparison.lastMonth.solarEarnings) *
                    100,
                }
              : undefined
      ...
      {stats.momComparison &&
        (() => {
          const momRows: MomComparisonRow[] = [
            ...
            {
              label: "Total Billed",
              lastMonthValue: stats.momComparison.lastMonth.billed,
              thisMonthValue: stats.momComparison.thisMonth.billed,
              format: "currency",
              invertColors: true,
            },
            {
              label: "Total Collected",
              lastMonthValue: stats.momComparison.lastMonth.collected,
              thisMonthValue: stats.momComparison.thisMonth.collected,
              format: "currency",
              invertColors: false,
            },
            ...(stats.momComparison.lastMonth.solarEarnings > 0 ||
            stats.momComparison.thisMonth.solarEarnings > 0
              ? [
                  {
                    label: "Solar Earnings",
                    lastMonthValue: stats.momComparison.lastMonth.solarEarnings,
                    thisMonthValue: stats.momComparison.thisMonth.solarEarnings,
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
        {stats.solarGenVsIncome.length > 0 ? (
          <>
            <ChartCard
              id="owner-cost-trend"
              title="Cost Trend (Bills vs Export)"
              data={stats.billsVsPaid.map((b) => ({
                month: b.month,
                billed: b.billed,
                exportEarnings:
                  stats.monthlyExportEarnings.find((e) => e.month === b.month)
                    ?.earnings || 0,
              }))}
            >
              {(slicedData) => (
            ...
            <ChartCard
              id="owner-solar-gen-income"
              title="Solar Gen vs Income"
              data={stats.solarGenVsIncome}
            >
              {(slicedData) => (
                <ResponsiveContainer
                  width="100%"
            ...
            <ChartCard
              id="owner-import-export"
              title="Import vs Export (kWh)"
              data={stats.importVsExport}
            >
              {(slicedData) => (
                <ResponsiveContainer
                  width="100%"
            ...
            <ChartCard
              id="owner-cumulative-profit"
              title="Cumulative Solar Profit"
              data={stats.cumulativeProfit}
            >
              {(slicedData) => (
                <ResponsiveContainer
                  width="100%"
          ...
          <>
            <ChartCard
              id="owner-bills-vs-paid"
              title="Billed vs Paid"
              data={stats.billsVsPaid}
            >
              {(slicedData) => (
                <ResponsiveContainer
                  width="100%"
            ...
            {stats.totalProperties > 1 && propertyNames.length > 0 && (
              <ChartCard
                id="owner-property-consumption"
                title="Property Consumption (kWh)"
          ...
          <DashboardCard className="overflow-hidden">
            <DataTable
              columns={[
                ...
                {
                  accessor: (
                    row: OwnerDashboardStats["outstandingBills"][number]
                  ) => (
                ...
                },
                {
                  accessor: (
                    ...
                    <button
                      ...
                      className="text-xs font-semibold px-3 py-1 rounded border border-primary text-primary hover:bg-primary hover:text-on-primary transition-colors duration-200"
                    >
                      Mark Paid
                    </button>
                  ),
