...
export function DashboardOverview() {
  ...
  const isEmptyOwner =
    ...
    (!ownerStats || ownerStats.totalProperties === 0);
  const isEmptyTenant =
    user?.primaryRole === "tenant" &&
    (!tenantStats || tenantStats.activeTenancies.length === 0) &&
    invites.length === 0;
  ...
  if (isEmptyOwner || isEmptyTenant) {
    return (
      <div>
        ...
        <p className="text-[0.9375rem] text-muted-foreground mb-10">
          Welcome to Meterly.
        </p>
        <div className="border border-border rounded-xl p-12 text-center max-w-lg mx-auto bg-surface">
          {user?.primaryRole === "tenant" ? (
            <>
              <p className="text-base font-semibold mb-2 font-heading">
                Waiting for an invite
              </p>
              <p className="text-[0.9375rem] text-muted-foreground m-0">
                Your landlord needs to invite you from their Meterly account.
              </p>
              <p className="text-[0.875rem] text-muted-foreground mt-6 mb-4 border-t border-border pt-6">
                Are you also a property owner?
              </p>
              <a
                href="/properties/new"
                className="btn btn-secondary inline-flex items-center text-sm font-semibold"
              >
                Add a property
              </a>
            </>
            ...
            <>
              ...
              <p className="text-[0.9375rem] text-muted-foreground mb-6">
                Add a property to start tracking meter readings and bills.
              </p>
              <a
                href="/properties/new"
                className="btn btn-primary inline-flex items-center text-sm font-semibold"
              >
    ...
    <div className="flex flex-col gap-10 max-w-6xl mx-auto pb-12">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground m-0 text-sm font-body">
            Your properties and performance at a glance.
          </p>
        </div>
        <a
          href="/properties/new"
          className="btn btn-primary no-underline text-sm font-semibold"
        >
          + Add Property
        </a>
      </div>
  ...
  return (
    <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      {invites.map((inv) => (
        <div
          key={inv.inviteToken}
          className="rounded-xl border border-border bg-surface p-5 border-l-4 border-l-accent"
        >
          <div className="flex items-start justify-between gap-2 mb-1">
            <p className="m-0 text-[0.9375rem] font-semibold text-foreground font-heading">
              {inv.propertyName}
            </p>
            <Badge variant="info">Invite</Badge>
          </div>
          {inv.propertyAddress && (
            <p className="mt-1 mb-0 text-[0.8125rem] text-muted-foreground">
              {inv.propertyAddress}
            </p>
          )}
          <a
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
        ...
        >
          ...
          <ul className="space-y-1 text-sm text-muted-foreground list-none p-0 m-0 mb-4">
            <li>
              {ownerStats.totalProperties} properties ·{" "}
              {ownerStats.activeTenants} tenants
            </li>
            <li>
              {formatCurrency(ownerStats.totalExportEarnings)} solar earnings
            </li>
            <li>
              {ownerStats.outstandingBills.length} unpaid bill
              {ownerStats.outstandingBills.length !== 1 && "s"}
            </li>
          </ul>
          <a
            href="/properties"
            className="text-sm font-semibold text-accent hover:text-accent/80 no-underline"
          >
            View Properties →
          </a>
        </div>
        ...
        >
          ...
          <ul className="space-y-1 text-sm text-muted-foreground list-none p-0 m-0 mb-4">
            {tenantStats.activeTenancies[0] ? (
              <>
                ...
                <li>
                  {tenantStats.currentBill?.totalDue
                    ? `Current bill: ${formatCurrency(tenantStats.currentBill.totalDue)}`
                    : "No current bill"}
                </li>
                {tenantStats.currentBill?.status && (
                  <li
                    className={
                      tenantStats.currentBill.status === "paid"
          ...
          {tenantStats.activeTenancies[0] && (
            <a
              href={`/tenancies/${tenantStats.activeTenancies[0].tenancyId}`}
              className="text-sm font-semibold text-accent hover:text-accent/80 no-underline"
            >
              View Tenancy →
            </a>
