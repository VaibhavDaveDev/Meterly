...
export function LoginForm({ turnstileSiteKey }: { turnstileSiteKey?: string }) {
  ...
  return (
    <AuthFormLayout
      ...
      footerLink={{ href: '/signup', text: "Don't have an account?", linkText: "Create account" }}
    >
      <form onSubmit={handleSignIn} method="post" className="flex flex-col gap-5 m-0">
        <EmailInput
          value={email}
          onChange={setEmail}
          disabled={isLoading}
        ...
        <PasswordInput
          ...
          rightLabel={
            <a
              ...
              className="text-sm font-medium text-accent hover:text-[#5558E3] transition-colors duration-200 ease-smooth"
            >
              Forgot password?
            </a>
          }
