...
export const meterReadingEdits = sqliteTable(
  "meter_reading_edits",
  {
    ...
    editedAt: integer("edited_at", { mode: "timestamp" }).default(
      sql`(strftime('%s', 'now'))`
    ),
  },
  (table) => [index("idx_meter_reading_edits_reading").on(table.meterReadingId)]
);
