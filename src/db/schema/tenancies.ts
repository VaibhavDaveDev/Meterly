...
export const tenancies = sqliteTable(
  "tenancies",
  {
    ...
    status: text("status", {
      enum: ["invited", "active", "inactive", "declined", "property_deleted"],
    }).notNull(),
    ...
    isOwnerTenancy: integer("is_owner_tenancy", { mode: "boolean" }).default(
      false
    ),
  ...
  (table) => [
    ...
    // Prevents two active tenancies for the same (tenant, property) pair.
    // NULL tenant_id rows (pending invites) are excluded.
    uniqueIndex("idx_tenancies_active_unique")
      .on(table.tenantId, table.propertyId)
      .where(sql`${table.status} = 'active' AND ${table.tenantId} IS NOT NULL`),
  ]
);
